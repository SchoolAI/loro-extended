...
import { SimplePeerDataChannelWrapper } from "../adapters/simple-peer-data-channel.js"
...
export function usePeerManager({
  ...
  const signalPeer = useCallback((remotePeerId: PeerID, signal: SignalData) => {
    ...
    if (connection) {
      try {
        // Our SignalData is a subset of simple-peer's SignalData type.
        // simple-peer also accepts transceiverRequest and renegotiate signals,
        // and uses RTCIceCandidate instead of RTCIceCandidateInit for candidates.
        // The cast is safe because simple-peer accepts all our signal types.
        connection.peer.signal(
