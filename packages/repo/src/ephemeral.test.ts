...
describe("Ephemeral Store Integration", () => {
  it("should allow setting and getting local ephemeral state", () => {
    ...
    expect(handle.ephemeral.self).toEqual({ cursor: { x: 10, y: 20 } })
  })
  ...
  it("should support the escape hatch setRaw", async () => {
    ...
    handle.ephemeral.setRaw("global-key", "global-value")

    expect(handle.ephemeral.all["global-key"]).toBe("global-value")
  })
