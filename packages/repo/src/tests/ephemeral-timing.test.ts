...
describe("Ephemeral Store - Timing Issues", () => {
  ...
  describe("Immediate presence setting", () => {
    it("should handle presence set immediately after getting document", async () => {
      const docId = "immediate-presence-doc"

      // Wait for connection
      await wait(100)

      // Simulate what React does: get document and immediately set presence
      const docA = clientA.get(docId, DocSchema, {
        presence: PresenceSchema,
      })
      sync(docA).presence.setSelf({ status: "online-A" })

      const docB = clientB.get(docId, DocSchema, {
        presence: PresenceSchema,
      })
      sync(docB).presence.setSelf({ status: "online-B" })

      // Wait for everything to sync
      await wait(200)

      // Both clients should see each other
      expect(sync(docA).presence.get("2")?.status).toBe("online-B")
      expect(sync(docB).presence.get("1")?.status).toBe("online-A")
    })
    ...
    it("should handle presence set before sync completes", async () => {
      ...
      // ClientA gets document and sets presence immediately (before sync)
      const docA = clientA.get(docId, DocSchema, {
        presence: PresenceSchema,
      })
      sync(docA).presence.setSelf({ status: "connecting" })
      ...
      sync(docB).presence.setSelf({ status: "connecting" })

      // Wait for sync
      await wait(200)

      // Both should see each other
      expect(sync(docA).presence.get("2")?.status).toBe("connecting")
      expect(sync(docB).presence.get("1")?.status).toBe("connecting")
    })
    ...
    it("should handle rapid presence updates", async () => {
      ...
      sync(docA).presence.setSelf({ status: "count-1" })
      await wait(50) // Small delay between updates
      sync(docA).presence.setSelf({ status: "count-2" })
      await wait(50)
      sync(docA).presence.setSelf({ status: "count-3" })

      // Wait for propagation
      await wait(200)

      // ClientB should see the final value
      expect(sync(docB).presence.get("1")?.status).toBe("count-3")
    })
  })
  ...
  describe("Staggered connection timing", () => {
    it("should propagate presence when second client connects later", async () => {
      ...
      sync(docA).presence.setSelf({ status: "early" })
      ...
      expect(sync(docB).presence.get("1")?.status).toBe("early")
    })
    ...
    it("should propagate presence from late joiner to early joiner", async () => {
      ...
      sync(docA).presence.setSelf({ status: "first" })
      ...
      sync(docB).presence.setSelf({ status: "second" })

      await wait(200)

      // Both should see each other
      expect(sync(docA).presence.get("2")?.status).toBe("second")
      expect(sync(docB).presence.get("1")?.status).toBe("first")
    })
  })
  ...
  describe("Subscription verification", () => {
    ...
    describe("Late Joiner Presence Visibility", () => {
      it("should propagate presence set BEFORE sync completes (React pattern)", async () => {
        ...
        sync(docA).presence.setSelf({ status: "Alice" })

        await wait(100)

        // Now simulate what React does: get document and IMMEDIATELY set presence
        const docB = clientB.get(docId, DocSchema, {
          presence: PresenceSchema,
        })
        ...
        const clientAPresenceOfB = sync(docA).presence.get("2")
