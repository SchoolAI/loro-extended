...
import { createPermissions } from "./permission-manager.js"
...
describe("Synchronizer - Refactored", () => {
  ...
  describe("sync functionality", () => {
    it("should trigger sync request", () => {
      const docId = "test-doc"
      synchronizer.getOrCreateDocumentState(docId)

      synchronizer.sync(docId)

      // Should generate patches from the sync operation
  ...
  describe("command execution", () => {
    it("should execute send-sync-response command", () => {
      ...
      // Should have sent a sync response
      expect(mockAdapter.sentMessages).toHaveLength(2) // establish-request + sync-response
    ...
    it("should handle establish channel doc command", () => {
      ...
      expect(updatedChannel?.shared.state).toBe("established")
    })
  ...
  describe("permissions integration", () => {
    it("should respect permissions in directory requests", () => {
      const restrictiveSync = new Synchronizer({
        ...
        adapters: [mockAdapter as AnyAdapter],
        permissions: createPermissions({
          canList: (_channelId, docId) => docId !== "secret-doc",
        }),
      })
  ...
  describe("patch generation", () => {
    ...
    it("should work without onPatch callback", () => {
      const syncWithoutPatch = new Synchronizer({
        identity: { name: "test" },
        adapters: [new MockAdapter("test") as AnyAdapter],
      })
