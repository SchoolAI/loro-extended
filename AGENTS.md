- To run all verifications including tests, use `pnpm verify`.
- If packages have been modified, build the package with `pnpm build` or `pnpm -w build` (for all packages).
- This is a monorepo. You can run commands inside a package via `pnpm --filter @loro-extended/[package-name]`. Example: `pnpm --filter @loro-extended/change -- verify`
...
pnpm --filter @loro-extended/repo -- verify logic
pnpm --filter @loro-extended/repo -- verify logic -- -t 'specific test'
